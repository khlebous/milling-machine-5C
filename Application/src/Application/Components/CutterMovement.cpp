...
bool Distance(sm::Vector3& pos1, DirectX::XMFLOAT4& pos2)
{
...
void CutterMovement::OnUpdate()
{
	...
	{
		if (!Distance(pos, instancedMesh->mInstancedData[i].ipos))
		{
			//mInstancedDatas.emplace_back(instancedMesh->mInstancedData[i]);
			DirectX::XMFLOAT4 tmp = instancedMesh->mInstancedData[i].ipos;
			sm::Vector3 point = sm::Vector3(tmp.x, tmp.y, tmp.z);
			if (!Distance(pos, pos1, point))
				mInstancedDatas.emplace_back(instancedMesh->mInstancedData[i]);
		}
	}
